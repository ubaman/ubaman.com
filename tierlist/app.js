(() => {
  ...
  function render() {
    ...
    const filtered = data.filter(champion =>
      (role === 'Todos' ? champion.best : champion.role === role) &&
      ...
      return `<section class="tier-row" data-tier="${tier}">
        ...
        <div class="champions">
          ${champions.map(champion => `<article class="champion">
            <span class="champion-mark"><img src="${safe(champion.portrait)}" alt="Retrato de ${safe(champion.name)}" width="64" height="64" loading="lazy" decoding="async" referrerpolicy="no-referrer"></span>
